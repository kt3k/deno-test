...
なお `@deno/loader` は Deno 公式がメンテナンスしているパッケージで、[`@deno/vite-plugin`](https://www.npmjs.com/package/@deno/vite-plugin) や [Fresh の Vite 統合 (`@fresh/plugin-vite`)](https://jsr.io/@fresh/plugin-vite) でも利用されています。Node / Vite エコシステムから Deno の解決ルールを使うための基盤として、すでに実戦投入されているライブラリです。
...
(なお、ライブラリ本体のコードについては、npm で配布する以上 Node.js の事情を考慮する必要があることには変わりません。`deno-test` が解決するのはあくまでテスト部分の開発体験です。)
...
* JSR パッケージをテストでそのまま使える

→ **JSR/Deno エコシステムをテストでそのまま使える**
